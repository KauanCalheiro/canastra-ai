...
Memórias e convenções do projeto ficam em `./memory/`. Antes de tomar decisões sobre algum tema abaixo, consulte o arquivo correspondente.
...
| [memory/frontend.md](memory/frontend.md) | Frontend (Nuxt) | Testes via Playwright em `./e2e` (fora do frontend); client nunca chama o backend direto — sempre via rota server-side proxy usando `canastraClient()` |
