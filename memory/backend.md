...
- **Action**: classe nomeada com o nome da ação (ex: `CreateGame`), **sem sufixo `Service`**. Fica em `app/Actions/<Domínio>/<Ação>.php` (namespace `App\Actions\<Domínio>`). Não usa `__invoke` — usa um método `handle()`.
  ...
  - **`make()` não recebe argumentos** (é só `app(static::class)`, conforme a própria trait). O Data de entrada é argumento do `handle(CreateGameData $data)`, nunca do construtor/`make()`. Uso: `CreateGame::make()->handle($data)` (ou `CreateGame::run($data)` como atalho).
...
Fluxo: `Controller::store(XData $data)` → `$game = Action::make()->handle($data)` → `XResource::make($game)`.
